...
void	parsing_out(t_pipex *p, char *outfile, char **all_paths)
{
	...
	if (p->cmd1_path && p->cmd2_path)
	{
		...
		{
			if (p->here_doc)
				p->fd_outfile = open(outfile, O_RDWR | O_APPEND | O_CREAT);
			else
			p->fd_outfile = open(outfile, O_RDWR | O_TRUNC | O_CREAT, 0777);
			if (p->fd_outfile == -1)
				perror(outfile);
		}
	}
...
{	
	...
	else if (!init_bonus(p, argc, argv, env))
		error_argument();
}
